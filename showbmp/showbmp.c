...
#include "sys/time.h"
#include "../libdlo-0.1.2/src/libdlo.h"
#include "../libdlo-0.1.2/src/dlo_defs.h"
 ...
 */
static bmp_t *load_bmp(const char * const filename)
{
  long int      size;
  bmp_t        *bmp;
  FILE         *fp;
...
static dlo_retcode_t bmp_clip_test(const dlo_dev_t uid)
{
  ...
  view[0]      = desc->view;
  view[1]      = view[0];
  view[2].base = view[0].base + (view[1].width * view[1].height * bpp_to_bytes(view[1].bpp));
  view[2]      = view[1];
  view[2].base = view[1].base + (view[2].width * view[2].height * bpp_to_bytes(view[2].bpp));
...
int main(int argc, char *argv[])
{
  ...
  if (uid)
  {
		...
		/* Clear screen */
		ERR(dlo_fill_rect(uid, NULL, NULL, DLO_RGB(0, 255, 255)));
		wait_ms(now(), 200);
		ERR(dlo_fill_rect(uid, NULL,     NULL, DLO_RGB(0, 0, 0xff)));

		char *bmpfile = "foo.bmp";//argv[1];
		/* Load the Windows BMP bitmap file into memory */
		printf("Loading %s\n", bmpfile);
		bmp = load_bmp(bmpfile);
		NERR(bmp);
		...
		// Read in file and disp to screen
		fbuf = bmp_to_fbuf(bmp);
    ERR_GOTO(dlo_copy_host_bmp(uid, flags, fbuf, NULL, &dot));
